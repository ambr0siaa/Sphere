...
#define COL_ASPECT (11.0f / 24.0f) // Ratio of symbols in console
...
{
    int color = 0;
    V3d n = v3d_norm(v3d(1.0, uv.x, uv.y)); // direction of ray
...
int main(void)
{
    ...
    V3d origin = { -2.0, 0.0, 0.0 }; // Origin of ray
    ...
    for (double t = 0.0; t < 2*PI; t += 0.01) {
        ...
        for (size_t i = 0; i < WIDTH; ++i) {
            for (size_t j = 0; j < HEIGHT; ++j) {
                V2d uv = v2d((double)(i) / (double)(WIDTH) * 2.0 - 1.0,
                             (double)(j) / (double)(HEIGHT) * 2.0 - 1.0);
                uv.x *= ASPECT * COL_ASPECT; // Aligns coordinates
